        ...
        <form method="post" action="add_t.php">
            <div class="input-group">
                ...
                <select name="add_task" id="add_task" class="custom-select">
                    ...
                    <option value="" disabled="disabled">----------</option>
                    <option value="st_acc.php">Statistic</option>
                    <option value="" disabled="disabled">----------</option>
...
<script>
    var countChecked = function() {
        ...
        var checked = $('#dr_table tbody').find('.chk:checked').length;

        // Total checkboxes excluding the "Select All" checkbox
        var total = $('#dr_table tbody').find('.chk').not(':first').length;
    ...
    $(document).on('change', '.chk', function() {
        countChecked(); // Recalculate on checkbox change
    });

    $(document).on('change', '.chk', function() {
        var result = countChecked($('#dr_table'), '.chk input[type="checkbox"]');
...
<script>
    $(document).ready(function () {
        ...
        // Функция начала перемещения окна
        $('.fixed-counter').on('mousedown', function (event) {
            isDragging = true;

            // Получаем начальное положение мыши относительно окна
            let rect = this.getBoundingClientRect();
            offset.x = event.clientX - rect.left;
            offset.y = event.clientY - rect.top;

            // Запрещаем выделение текста при перемещении
        ...
        $(document).on('mousemove', function (event) {
            if (isDragging) {
                ...
                $('.fixed-counter').css({ left: newX + 'px', top: newY + 'px' });
            }
        });

        // Функция завершения перемещения окна
